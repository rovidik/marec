(function () {
    ...
    var countryNames = {
        ...
        'vn': 'Вєтнам', 'ae': 'ОАЕ', 'sa': 'Саудівська Аравія', 'eg': 'Єгипет'
    };
   ...
   function renderStudiosTitle(render, title, movie) {
    ...
    function getBestAndPopular(results, movie) {
        ...
        var ukrPattern = /(ukr|ua|ukrainian|українська|укр|[^a-z]uk[^a-z])/i;
        var ukrResults = [];
        ...
        results.forEach(function(item) {
            ...
            var titleClean = title.replace(/(укр[а-яієїґ]*|ukr[a-z]*|ua|ukrainian)[\s\.\,\_\-\|]*(sub|суб)[a-zа-яієїґ]*/ig, '').replace(/(sub|суб)[a-zа-яієїґ]*[\s\.\,\_\-\|]*(укр[а-яієїґ]*|ukr[a-z]*|ua|ukrainian)/ig, '');
