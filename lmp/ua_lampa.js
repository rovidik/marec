(function () {
    ...
    function createV2(sourceKey) {
        return function v2(component, _object) {
            ...
            this.search = function (object, data) {
                ...
                url = Lampa.Utils.addUrlComponent(url, 'title=' + encodeURIComponent(first.title || object.movie.title || object.movie.name));
                ...
                network.silent(url, function (json) {
                    ...
                    // Додаємо позначку [UA] та сортуємо
    ...
    // РЕЄСТРАЦІЯ ПЛАГІНА ЯК ОБ'ЄКТА
    Lampa.Plugins.add({
        id: 'makhno_ua_plugin',
        name: 'Makhno [UA]',
        description: 'Українські джерела з пріоритетом UA',
        version: '1.0.1',
        ready: function () {
            // Додаємо компонент
            Lampa.Component.add('makhno_ua', createV2('makhno'));

            // Додаємо в список джерел (Парсер)
            ...
            console.log('Makhno [UA]: Плагін готовий');
        }
    });
})();