(function() {
    ...
    if (window.plugin_cinemas_ready) return;
    window.plugin_cinemas_ready = true;

    // Додаємо переклади
    if (window.Lampa && window.Lampa.Lang) {
        Lampa.Lang.add({
            menu_cinemas: { ru: 'Источники', en: 'Sources', uk: 'Джерела' },
            title_cinemas: { ru: 'Источники', en: 'Sources', uk: 'Джерела' }
        });
    }

    function addMenuItem() {
        // ПЕРЕВІРКА: якщо кнопка з data-action="cinemas" вже є, нічого не робимо
        if ($('.menu .menu__list [data-action="cinemas"]').length > 0) return;

        var menu_item = $('<div class="menu__item selector" data-action="cinemas">' +
            '<svg height="36" viewBox="0 0 24 24" width="36" xmlns="http://www.w3.org"><path d="M0 0h24v24H0z" fill="none"/><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-5 14H4v-4h11v4zm0-5H4V9h11v4zm5 5h-4V9h4v9z" fill="currentColor"/></svg>' +
            '<div class="menu__text">' + Lampa.Lang.translate('menu_cinemas') + '</div>' +
        '</div>');

        menu_item.on('hover:enter', function () {
            Lampa.Activity.push({
                url: '',
                title: Lampa.Lang.translate('title_cinemas'),
                component: 'cinemas',
                page: 1
            });
        });

        // Додаємо в кінець першого списку меню
        $('.menu .menu__list').first().append(menu_item);
    }

    // Запуск додавання в меню
    if (window.appready) {
        addMenuItem();
    } else {
    ...
    // Не забудьте додати порожній компонент, щоб при натисканні не було помилки
    Lampa.Component.add('cinemas', function (object) {
        ...
        this.destroy = function () {};
            ...
            sort_popular: { ru: 'Популярные', en: 'Popular', uk: 'Популярні' },
            sort_rating_desc: { ru: 'Рейтинг', en: 'Rating', uk: 'Рейтинг' },
            sort_date_desc: { ru: 'Новые', en: 'Newest', uk: 'Нові' },
            sort_date_asc: { ru: 'Старые', en: 'Oldest', uk: 'Старі' },
            sort_vote_count: { ru: 'По голосам', en: 'Most voted', uk: 'За голосами' },

            cat_global_charts: { ru: 'Чарты', en: 'Charts', uk: 'Чарти' },
            cat_streaming: { ru: 'Стриминги', en: 'Streaming', uk: 'Стрімінги' },
            cat_us_tv: { ru: 'TV США', en: 'US TV', uk: 'TV США' },
            cat_world_tv: { ru: 'TV Мир', en: 'World TV', uk: 'TV Світ' },
            cat_ua: { ru: 'Украина', en: 'Ukraine', uk: 'Україна' },
            cat_ru: { ru: 'Россия', en: 'Russia', uk: 'Росія' },

            cat_popular: { ru: 'Популярные', en: 'Popular', uk: 'Популярні' },
            cat_top_rated: { ru: 'Топ-250', en: 'Top 250', uk: 'Топ-250' },
            cat_airing_today: { ru: 'Сегодня в эфире', en: 'Airing today', uk: 'Сьогодні в ефірі' },
            cat_on_the_air: { ru: 'Скоро', en: 'Upcoming', uk: 'Незабаром' },
            cat_most_voted: { ru: 'Самые обсуждаемые', en: 'Most voted', uk: 'Найбільше голосів' },
            cat_trending: { ru: 'В тренде', en: 'Trending', uk: 'У тренді' }
        });
    }
    ...
    var categories = [
        {
            id: 'global',
            key: 'cat_global_charts',
            list: [
                { key: 'cat_popular', type: 'popular', endpoint: 'tv/popular' },
                { key: 'cat_top_rated', type: 'top_rated', endpoint: 'tv/top_rated' },
                { key: 'cat_airing_today', type: 'airing_today', endpoint: 'tv/airing_today' },
                { key: 'cat_on_the_air', type: 'on_the_air', endpoint: 'tv/on_the_air' },
                { key: 'cat_most_voted', type: 'most_voted', endpoint: 'discover/tv', sort: 'vote_count.desc' },
                { key: 'cat_trending', type: 'trending', endpoint: 'trending/tv/week' }
            ]
        },
        {
            id: 'streaming',
    ...
    var sortOptions = [
        { key: 'sort_popular', sort: 'popularity.desc' },
        { key: 'sort_rating_desc', sort: 'vote_average.desc' },
        { key: 'sort_date_desc', sort: 'first_air_date.desc' },
        { key: 'sort_date_asc', sort: 'first_air_date.asc' },
        { key: 'sort_vote_count', sort: 'vote_count.desc' }
    ];
    ...
    function chooseSort(net) {
        if (net.type) {
            ...
            Lampa.Select.show({
                title: safeTranslate('title_sort'),
                items: items,
    ...
    function chooseCategory() {
        ...
        Lampa.Select.show({
            title: safeTranslate('title_cinemas'),
            items: items,
            ...
            if ($menu.length) {
                try {
                    var button = $(
                        ...
                        '    </svg>' +
                        '  </div>' +
                        '  <div class="menu__text">' + safeTranslate('menu_cinemas') + '</div>' +
                        '</li>'
                    );
