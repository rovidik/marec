(function() {
    ...
    // 1. Безпечна перевірка на дублювання плагіна
    if (window.plugin_cinemas_ready) return;
    window.plugin_cinemas_ready = true;

    // 2. Безпечна перевірка наявності об'єкта Lampa через window
    if (window.Lampa && window.Lampa.Lang && typeof window.Lampa.Lang.add === 'function') {
        Lampa.Lang.add({
            menu_cinemas: { ru: 'Источники', en: 'Sources', uk: 'Джерела' },
            title_cinemas: { ru: 'Источники', en: 'Sources', uk: 'Джерела' },
            title_sort: { ru: 'Сортировка', en: 'Sort', uk: 'Сортування' },
